# project_guardian/tests/test_memory_review_recovery_audit_tamper_evidence.py
...
SCRIPT_PATH = (
    REPO_ROOT / "scripts" / "run_memory_review_recovery_audit_tamper_evidence_smoke.py"
)


def _load_smoke_module():
    module_name = "run_memory_review_recovery_audit_tamper_evidence_smoke"
...
def _valid_entry(**overrides: Any) -> Dict[str, Any]:
    entry = {
        "recovery_event_id": "0123456789abcdef0123456789abcdef",
        "event_type": "corruption_detected",
        "event_at": "2026-01-01T00:00:00+00:00",
        "source_log_path": "/tmp/review_decisions.jsonl",
        "quarantine_log_path": "",
        "quarantine_manifest_path": "",
        "detected_error_types": ["malformed_json"],
...
def test_recovery_audit_tamper_smoke_json_cli_passes():
    payload = _run_script()
    ...
    assert payload["missing_required_field_detected"] is True
    assert payload["non_chronological_detected"] is True
    assert payload["unsupported_event_detected"] is True
...
def test_recovery_audit_tamper_json_contract_fields_present():
    payload = _run_script()
    required_fields = {
        "verdict",
        "workspace",
        ...
        "missing_required_field_detected",
        "non_chronological_detected",
        "unsupported_event_detected",
...
def test_recovery_audit_tamper_each_case_returns_fail_with_specific_token():
    payload = _run_script()
...
def test_checker_clean_recovery_audit_log_passes(tmp_path):
    log = _write_log(
        tmp_path / "clean.jsonl",
        _valid_entry(),
        _valid_entry(
            event_type="quarantine_created",
            event_at="2026-01-01T00:00:01+00:00",
        ),
    )
    verdict, errors = SMOKE.audit_recovery_log(log)
    assert verdict == "PASS"
    assert errors == []


def test_checker_detects_malformed_json(tmp_path):
    log = _write_log(tmp_path / "malformed.jsonl", _valid_entry(), raw_lines=["{ not valid json"])
    verdict, errors = SMOKE.audit_recovery_log(log)
    assert verdict == "FAIL"
...
def test_checker_detects_non_chronological(tmp_path):
    log = _write_log(
        ...
        _valid_entry(event_at="2026-06-01T00:00:00+00:00"),
        _valid_entry(event_at="2020-01-01T00:00:00+00:00"),
    )
    verdict, errors = SMOKE.audit_recovery_log(log)
    assert verdict == "FAIL"
...
def test_checker_detects_unsupported_event(tmp_path):
    log = _write_log(tmp_path / "unsupported.jsonl", _valid_entry(event_type="invalid_recovery_action"))
    verdict, errors = SMOKE.audit_recovery_log(log)
    assert verdict == "FAIL"
    assert any("unsupported_event" in err for err in errors)


def test_checker_detects_unsafe_metadata(tmp_path):
    log = _write_log(tmp_path / "unsafe.jsonl", _valid_entry(dry_run=False))
    verdict, errors = SMOKE.audit_recovery_log(log)
    assert verdict == "FAIL"
    assert any("unsafe_metadata" in err for err in errors)


def test_recovery_audit_tamper_script_has_no_network_ui_or_route_behavior():
...
def test_recovery_audit_tamper_script_refuses_repository_root_workspace():
    result = subprocess.run(
