...
def test_conflicting_duplicate_identity_fails_closed_even_across_scope():
    result = aggregate_verdict([vr("same-id", CURRENT_SHA, "#39", Verdict.PASS), vr("same-id", CURRENT_SHA, "#40", Verdict.PASS)], product_sha=CURRENT_SHA, contract="#39")
...
def test_permutation_and_replay_are_deterministic():
    ...
    assert aggregate_verdict(list(expected.history), product_sha=CURRENT_SHA, contract="#39") == expected
