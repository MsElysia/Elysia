...
class VerdictRecord:
    """A verifier record whose admission occurred outside this module."""
...
@dataclass(frozen=True, slots=True)
class AuditRecord:
    """Preserved, routing-ineffective unsupported control record."""
...
def _record_key(record: Record) -> tuple[str, str, str, str, str, str]:
    if isinstance(record, VerdictRecord):
        return (
            ...
            "verdict",
            record.verifier_id,
            f"{record.verdict.value}:{record.evidence_ref}",
        )
    return (
        record.product_sha,
        record.contract,
        ...
        "audit",
        record.action,
        f"{record.target_record_id}:{record.evidence_ref}",
    )


def _validate_record(record: Record) -> str | None:
    # The type check is the trust boundary: raw mappings cannot self-assert admission.
...
def aggregate_verdict(
    records: Iterable[Record], *, product_sha: str, contract: str
) -> AggregationResult:
    """Reduce pre-admitted records for exactly one product SHA and contract.

    FAIL dominates PASS.  PASS dominates later PENDING/NEEDS_REVIEW omissions.
    Unsupported invalidation-like audit records are retained but routing-ineffective.
    Any malformed input or conflicting reuse of a stable record id fails closed.
    """
    ...
    history = tuple(sorted(materialized, key=_record_key)) if not any(
        type(r) not in (VerdictRecord, AuditRecord) for r in materialized
    ) else tuple(materialized)

    if reasons:
        return AggregationResult(
            product_sha, contract, RoutingState.FAIL_CLOSED, True, history, tuple(sorted(set(reasons)))
        )

    scoped = [
        r for r in materialized
        if isinstance(r, VerdictRecord)
        and r.product_sha == product_sha
...
def aggregate_candidate_gate(
    technical_results: Iterable[AggregationResult], *, human_governance_required: bool
) -> str:
    """Keep whole-candidate human governance separate and dominant."""
