...
def test_accept_requires_live_owner_and_records_decision(tmp_path):
    ...
    try:
        ...
        assert not ledger.accept_verification("write-1", "verifier-b", ["artifact:test", "review:wrong"], now=NOW)
        assert not ledger.accept_verification("write-1", "verifier-a", ["artifact:test", "review:stale"], now=NOW + timedelta(seconds=61))
        ...
        assert ledger.accept_verification("write-1", "verifier-b", ["artifact:test", "review:ok"], now=NOW + timedelta(seconds=63))
        row = ledger.conn.execute("SELECT * FROM tasks WHERE task_id='write-1'").fetchone()
